...
const HIGHLIGHT_RE = /\b(?:hot food|boxed meals|salsa|coffee|hh|alcohol|bevs?|snack packs?)\b/gi;
...
// A row is a pickup if its OWN values say so — Delivery Group literally
// "Pickup", or Departure Time literally "N/A" — mirrors api/pipeline.py's
// _classify_row() exactly. Used when the user edits Group/Departure
// directly in the review table, so a manual correction (e.g. typing
// "Pickup" into Delivery Group) reclassifies the row live, the same way the
// backend would on a fresh parse.
const PICKUP_GROUP_RE = /^pickup$/i;
const NA_RE = /^n\/?a$/i;

export function classifyRowType(group, departure) {
  if (PICKUP_GROUP_RE.test((group || '').trim())) return 'pickup';
  if (NA_RE.test((departure || '').trim())) return 'pickup';
  return 'data';
}
